...
extension TokenAccountSupportCatalog {
    static let supportByProvider: [UsageProvider: TokenAccountSupport] = {
        var support: [UsageProvider: TokenAccountSupport] = [
        ...
        // Kill switch: defaults write <bundle-id> codexMultiAccountEnabled -bool NO
        // Defaults to enabled to avoid requiring migrations.
        let codexMultiAccountEnabled = UserDefaults.standard.object(forKey: "codexMultiAccountEnabled") as? Bool ?? true
        if codexMultiAccountEnabled {
            support[.codex] = TokenAccountSupport(
                title: "Codex accounts",
                subtitle: "Paste the contents of ~/.codex/auth.json after each `codex login`.",
                placeholder: "Paste auth.json contents… or use Import current login",
