...
Task 0 runs first and is authoritative. It supersedes any conflicting original execution instructions below. Tasks 1–7 are run only as ordered verification/reconciliation tasks on `codex/upstream-parity-recovery`; their original RED-test, create-test, and source-replay commit snippets are historical records of work already present on the preserved source branch and must not be executed again. Any needed conflict fix or missing coverage is committed atomically from the integration worktree only. Task 8 Step 6 below is replaced by its amended fork-scoped command; the original source-branch PR command is not executable.
...
Every task commit below must also perform the repository-required propagation and documentation pass before staging: search all `Sources`, `Tests`, and `docs` references to each changed symbol/configuration key; update the task's listed documentation plus `docs/knowledge/timeline/2026-07-11-cursor-opencode-parity.md`; run `git diff --check`; print `[DOCS UPDATED: ...]`; and stage only the files named by that task. If a task is test-only, its propagation search and timeline entry still record the regression contract. After every green/reconciled task that edits code or documentation, run `CODEXBAR_SIGNING=adhoc ./Scripts/compile_and_run.sh --wait` from the integration worktree and verify the launched bundle remains alive before committing that task. The final code-review fixes follow the same gate in their own atomic commit.

### Task 0: Integrate the preserved upstream replay into the fork

1. Run `git fetch --prune origin main` and verify `origin` is `https://github.com/ivalsaraj/CodexBar.git`.
2. Create the isolated integration worktree `/Users/valsaraj/.codex/worktrees/cursor-upstream-parity-recovery` on branch `codex/upstream-parity-recovery` from the current `origin/main`.
3. Merge `codex/cursor-upstream-replay-before-user-main-merge` into the integration branch. Resolve conflicts by retaining the complete upstream implementation and every plan requirement/exclusion; do not reduce the result to the prior five-file patch.
4. Verify the preserved source tip is an ancestor of the completed merge and that the merge includes the full source ancestry, not only Cursor/OpenCode files.
5. Keep the source branch/worktree and the integration worktree until the final PR is merged and post-merge verification completes.

Tasks 1–7 below run only as ordered verification/reconciliation tasks on `codex/upstream-parity-recovery`. Their original RED-test, create-test, and source-replay commit snippets are historical records of work already present on the preserved source branch and must not be executed again. Any needed conflict fix or missing coverage is committed atomically from the integration worktree only.
...
Expected: the source worktree remains preserved and read-only; all implementation, reconciliation, staging, and commits run only in `/Users/valsaraj/.codex/worktrees/cursor-upstream-parity-recovery`; do not stage or edit the dirty custom workspace.
...
**Step 6: Push and open the pull request (integration branch only)**

This replaces the original source-branch command block. Before opening the PR, generate `/tmp/cursor-opencode-parity-pr.md` from the final user-visible summary, all checks run, Codex plan/code approval statuses, and remaining risks. Verify the integration worktree is clean, then run:

```bash
git fetch --prune origin main
git status --short --branch
git push -u origin codex/upstream-parity-recovery
gh pr create --repo ivalsaraj/CodexBar --base main --head codex/upstream-parity-recovery --title "Restore Cursor diagnostics and OpenCode workspace parity" --body-file /tmp/cursor-opencode-parity-pr.md
```

Capture the resulting PR URL and confirm its repository and base are `ivalsaraj/CodexBar:main`. The source branch and worktree remain preserved until the PR is merged and post-merge verification completes.
