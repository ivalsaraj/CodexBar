...
struct OpenCodeStatusMenuTests {
    ...
    @Test
    func `returning to active workspace preview keeps cached usage visible`() throws {
        StatusItemController.menuCardRenderingEnabled = false
        StatusItemController.menuRefreshEnabled = false

        let suite = "OpenCodeStatusMenuTests-active-preview-\(UUID().uuidString)"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        let configStore = testConfigStore(suiteName: suite)
        ...
        let config = CodexBarConfig(providers: [
            ProviderConfig(
                ...
                openCodeWorkspaceAccounts: OpenCodeWorkspaceAccountData(
                    version: 1,
                    activeAccountID: firstWorkspaceAccountID,
                    accounts: [
                        OpenCodeWorkspaceAccount(
                            id: firstWorkspaceAccountID,
                            tokenAccountID: tokenAccountID,
        ...
        store.snapshots[.opencode] = snapshot
        controller.tokenAccountPreviewSelection[.opencode] = firstWorkspaceAccountID

        let model = try #require(controller.menuCardModel(for: .opencode))
